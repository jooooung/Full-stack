...
-- 2. 글목록 (topN구문)
SELECT ROWNUM RN, A.* FROM (SELECT * FROM BOARD ORDER BY REF DESC, REF_STEP) A;
SELECT * 
...
INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, REF_STEP, REF_INDENT, IP) 
    VALUES (89, '홍', '원글', 'ㅁㅁ', NULL, '111', 89, 0, 0, '123.1.1.1');
-- 89번글 1번 답변글
  -- 8. 답변글 추가 전단계(a단계 - 원글의 REF:89, 원글의 REF_STEP:1 )
  UPDATE BOARD SET REF_STEP = REF_STEP + 1
    WHERE REF = 89 AND REF_STEP > 4;  

-- 최고 글 번호
SELECT MAX(NUM) FROM BOARD;

  -- 9. 답변글 INSERT (원글REF = 89, 원글REF_STEP=1, 원글REF_INDENT=1)
  INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, REF_STEP, REF_INDENT, IP) 
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '홍홍', '답글1', 'ㅁㅁ', NULL, '111', 89, 1, 1, '123.1.1.1');
    
  -- 9-1. 답변글의 답변글 INSERT (원글REF = 89, 원글REF_STEP=1, 원글REF_INDENT=1)
  INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, REF_STEP, REF_INDENT, IP) 
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '홍홍', '답글1-1', 'ㅁㅁㅁ', NULL, '111', 89, 5, 3, '123.1.1.1');
    
-- 89번글 2번 답변글
    UPDATE BOARD SET REF_STEP = REF_STEP + 1
    WHERE REF = 89 AND REF_STEP > 0;
  INSERT INTO BOARD (NUM, WRITER, SUBJECT, CONTENT, EMAIL, PW, REF, REF_STEP, REF_INDENT, IP) 
    VALUES ((SELECT NVL(MAX(NUM), 0)+1 FROM BOARD), '홍홍홍', '답글2', 'ㅁㅁㅁ', NULL, '111', 89, 1, 1, '123.1.1.1');
-- 
COMMIT;


SELECT * FROM BOARD WHERE REF=(SELECT REF FROM BOARD WHERE NUM =91);